...
SELECT *
FROM Cars
ORDER BY ABS(Price - ROUND((SELECT AVG(Price) FROM Cars), -2))
LIMIT 1;
...
WHERE Price > (
    SELECT AVG(Price)
    FROM Cars
    WHERE TransmissionType = c.TransmissionType
      AND FuelType = c.FuelType
);

-- 10.	Find the brand with the highest total mileage across all models.
SELECT Brand, SUM(Mileage) AS Total_Mileage
FROM Cars
GROUP BY Brand

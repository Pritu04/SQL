-- SQL PROJECT: Employees & Departments Database
...
-- 9. DELETE QUERIES
-- 10. SCHEMA MODIFICATION QUERIES
...
ALTER TABLE Departments ADD EstablishedYear INT;
ALTER TABLE Departments CHANGE Location City VARCHAR(50);
ALTER TABLE Departments MODIFY DepartmentName VARCHAR(100);
ALTER TABLE Departments DROP COLUMN HeadOfDepartment;
...
SELECT DepartmentID, MIN(DateOfJoining) AS EarliestJoinDate FROM Employees GROUP BY DepartmentID;
...
SELECT DepartmentID, COUNT(*) AS JoinedIn2024 FROM Employees WHERE YEAR(DateOfJoining) = 2024 GROUP BY DepartmentID;
...
SELECT DepartmentID, COUNT(*) AS MidLevelCount FROM Employees WHERE Salary BETWEEN 50000 AND 70000 GROUP BY DepartmentID;


-- Situation -10: For diversity analysis, count the number of employees whose names start with 
-- each letter of the alphabet.

SELECT LEFT(FirstName,1) AS FirstLetter, COUNT(*) AS CountByLetter FROM Employees GROUP BY LEFT(FirstName,1) ORDER BY FirstLetter;

-- Department Table 
-- Situation -11: For city-based analysis, count the number of departments located in each city. 

SELECT Location, COUNT(*) AS DeptCount FROM Departments GROUP BY Location;
...
-- manager.
 ...
 SELECT HeadOfDepartment, COUNT(*) AS DeptCount FROM Departments GROUP BY HeadOfDepartment;
 
 
-- Situation -14: To plan budget allocation, calculate the average budget of departments in each 
-- city. 

SELECT Location, AVG(AnnualBudget) AS AvgBudget FROM Departments GROUP BY Location;


-- Situation -15: For funding efficiency, find the maximum and minimum budgets among 
-- departments in each city.
 
SELECT Location, MAX(AnnualBudget) AS MaxBudget, MIN(AnnualBudget) AS MinBudget FROM Departments GROUP BY Location;
...
SELECT CONCAT(DepartmentName, ' - ', Location) AS DeptAddress FROM Departments;
...
SELECT CONCAT(DepartmentID, '-', EmployeeID) AS UniqueID FROM Employees;


SELECT CONCAT(FirstName, LastName, DepartmentID) AS LoginID FROM Employees;
SELECT CONCAT(YEAR(DateOfJoining), '-', FirstName) AS ReportRef FROM Employees;
...
UPDATE Employees
SET PhoneNumber = '9876543210', Extension = '101'
WHERE EmployeeID = 1;

UPDATE Employees
SET PhoneNumber = '9123456789', Extension = NULL
WHERE EmployeeID = 2;
...
-- In  current Employees table, these columns do not exist:
-- Street
-- City
-- PostalCode
-- So we first add them
...
-- and their department ID. 
     ...
     -- This one fits perfectly with your existing table, so NO schema change is needed.
     -- Requirement:
     -- Create an Employee Login ID by concatenating:
	...
	-- Last Name
     ...
     SELECT 
EmployeeID,
CONCAT(FirstName, LastName, DepartmentID) AS EmployeeLoginID
FROM Employees;

-- Alternative (With Separator – Optional but Professional)
SELECT 
EmployeeID,
CONCAT(FirstName, '.', LastName, '_', DepartmentID) AS EmployeeLoginID
FROM Employees;

     -- ONE-LINE EXAM ANSWER
     SELECT CONCAT(FirstName, LastName, DepartmentID) AS EmployeeLoginID FROM Employees;


-- Situation -8: For sending out a personalized notification, concatenate the employee's department 
-- with their position. 

-- In the current project schema, the Employees table does NOT contain a Position column.
-- So we add it (this is acceptable and expected in real projects).
...
SELECT 
EmployeeID,
CONCAT('Department ', DepartmentID, ' - ', Position) AS NotificationMessage
FROM Employees;

SELECT 
E.EmployeeID,
CONCAT(D.DepartmentName, ' - ', E.Position) AS NotificationMessage
FROM Employees E
JOIN Departments D
ON E.DepartmentID = D.DepartmentID;

-- “Since position information was not available in the original table, I added a Position column.
-- Then I used CONCAT to combine the department name with the employee’s position to generate a personalized notification.”
SELECT CONCAT(D.DepartmentName, ' - ', E.Position)
FROM Employees E
JOIN Departments D
ON E.DepartmentID = D.DepartmentID;
...
-- project. 
-- Requirement:
-- Display the employee’s work status by concatenating:

-- Job Title

-- Current Project

-- In the existing Employees table, these columns do NOT exist:
...
SELECT 
EmployeeID,
CONCAT(YEAR(DateOfJoining), '_', FirstName) AS ReportReferenceCode
FROM Employees;

-- ONE-LINE EXAM ANSWER

SELECT CONCAT(YEAR(DateOfJoining), '_', FirstName) AS ReportReferenceCode FROM Employees;
...
UPDATE Employees SET Salary = Salary * 0.10 WHERE DepartmentID = 3;
...
-- accordingly and make sure the position is reflected correctly in the employee records. 
   ...
   -- Requirement:
   -- Employee with EmployeeID = 101 is promoted to “Senior Developer”.
   -- Update their job title in the employee records.
   -- In the original Employees table, there is no JobTitle / Position column.
   -- So we add a JobTitle column first
   ALTER TABLE Employees
ADD JobTitle VARCHAR(50);
...
-- communications.
       ...
       -- Update the contact number of employee with EmployeeID = 120 to 9876543210.
       -- In your original Employees table, there is no PhoneNumber / ContactNumber column.
       -- So we first add it to store contact details.
...
SELECT EmployeeID, FirstName, Email, DateOfJoining
FROM Employees
WHERE DateOfJoining > '2023-01-01';

-- ONE-LINE EXAM ANSWER
UPDATE Employees SET Email = REPLACE(Email,'@example.com','@newcompany.com')
WHERE DateOfJoining > '2023-01-01';
...
-- Please update the project assignment for this employee to reflect this new responsibility.
     ...
     -- EmployeeID = 110
      ...
      -- Assign Current Project → "Project Alpha"
      -- In your original Employees table, there is no column to store project information.
      -- So we add a CurrentProject column.
      ALTER TABLE Employees
ADD CurrentProject VARCHAR(50);
...
SELECT EmployeeID, FirstName, CurrentProject
FROM Employees
WHERE EmployeeID = 110;
...
-- Delete the employee record with EmployeeID 101
DELETE FROM Employee
WHERE EmployeeID = 101;

DELETE FROM Employees WHERE EmployeeID = 5;
DELETE FROM Employees WHERE DateOfJoining < '2023-01-01';
DELETE FROM Employees WHERE Salary < 40000;
DELETE FROM Employees WHERE Email IS NULL OR Email = '';
DELETE FROM Employees WHERE DepartmentID = 2;
DELETE FROM Departments WHERE DepartmentID = 11;
DELETE FROM Departments WHERE Location = 'Pune';
DELETE FROM Departments WHERE AnnualBudget < 700000;
DELETE FROM Departments WHERE HeadOfDepartment = 'Rakesh Mehta';
...
-- active projects should be removed from the system. Delete these employee records. 
      ...
      -- In the original Employees table, there is no column to track project status.
      -- So we assume:

	  -- A column named CurrentProject

       -- If CurrentProject is NULL, the employee has no active project
-- Verify Before Deleting 
SELECT EmployeeID, FirstName, DateOfJoining, CurrentProject
FROM Employees
WHERE DateOfJoining < '2020-01-01'
  AND CurrentProject IS NULL;
  
  
-- step1: Add Project Column
ALTER TABLE Employees
ADD CurrentProject VARCHAR(50); -- Skip this step if CurrentProject already exists.

--  step2: MAIN DELETE QUERY
DELETE FROM Employees
WHERE DateOfJoining < '2020-01-01'
  AND CurrentProject IS NULL;
...
-- were working on this project (assumed to be recorded in ProjectName). 
      ...
      -- In the original Employees table, there is no column for project name.
      -- So we assume a column called ProjectName exists (or add it).
      
     -- STEP 1: Add ProjectName Column (If Not Already Added)
ALTER TABLE Employees
ADD ProjectName VARCHAR(50);  -- Skip this step if ProjectName already exists.
...
WHERE DepartmentID = 3
  ...
  SELECT EmployeeID, FirstName, DepartmentID, DateOfJoining
FROM Employees
WHERE DepartmentID = 3
  AND DateOfJoining < '2021-01-01';
...
-- have not provided their email addresses or contact numbers. 
   -- Employees table does NOT have a PhoneNumber column But the DELETE query is trying to use it
   
       -- First add the missing column, then delete
       -- Step 1: Add PhoneNumber column
ALTER TABLE Employees
ADD PhoneNumber VARCHAR(15);

SELECT EmployeeID, FirstName, Email, PhoneNumber
FROM Employees
WHERE Email IS NULL
   OR PhoneNumber IS NULL;


       -- Step 2: Run the DELETE query again
...
-- a temporary contract that has now expired. Delete these employee records. 
     ...
     -- (In the original Employees table, there are no columns to store: Contract type Contract status / expiry So we assume: 
     -- EmployeeCategory → stores 'Temporary' ContractStatus → stores 'Expired')
     
     -- STEP 1: Add Required Columns (If Not Already Added)
ALTER TABLE Employees
ADD EmployeeCategory VARCHAR(20),
ADD ContractStatus VARCHAR(20);

     -- STEP 2: MAIN DELETE QUERY
 
 DELETE FROM Employees
WHERE Salary > 100000
  AND EmployeeCategory = 'Temporary'
  AND ContractStatus = 'Expired';
  
  SELECT EmployeeID, FirstName, Salary, EmployeeCategory, ContractStatus
FROM Employees
WHERE Salary > 100000
  AND EmployeeCategory = 'Temporary'
  AND ContractStatus = 'Expired';
...
-- exists), and whose employment ended before 2023.
        ...
        -- (In the original Employees table, these columns do not exist: Region EmploymentEndDate So we assume:
         -- Region → stores employee work region (e.g., North, South) EmploymentEndDate → stores the date when employment ended)

        -- STEP 1: Add Required Columns (If Not Already Added)
        
ALTER TABLE Employees
ADD Region VARCHAR(20),
ADD EmploymentEndDate DATE;

SELECT EmployeeID, FirstName, Region, EmploymentEndDate
FROM Employees
WHERE Region = 'North'
  AND EmploymentEndDate < '2023-01-01';

          -- STEP 2: MAIN DELETE QUERY (PROJECT-READY)
DELETE FROM Employees
WHERE Region = 'North'
  AND EmploymentEndDate < '2023-01-01';
...
PARTITION BY RANGE (YEAR(DateOfJoining)) (
    ...
    PARTITION p_2020_onwards VALUES LESS THAN MAXVALUE
                  ...
                  -- -- MySQL does not support partitioning on tables with foreign keys.-- -- Hence, Employees table cannot be partitioned unless the foreign key is removed.
        -- Verify Partitions
...
WHERE TABLE_NAME = 'Employees';
      ...
      -- ------- Or ------------
      -- Use Archive Table Instead of Partitioning

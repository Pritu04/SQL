...
WHERE DepartureCity='New York'
  AND DepartureTime > '2023-01-30 00:00:00';
...
WHERE AvailableSeats < 150
  AND DepartureTime > '2023-02-20 00:00:00';
...
SELECT ArrivalCity, COUNT(*) AS TotalFlights
FROM Flights
GROUP BY ArrivalCity
HAVING COUNT(*) > 2;
...
SELECT ArrivalCity, AVG(TicketPrice) AS AvgPrice
FROM Flights
GROUP BY ArrivalCity
...
SELECT ArrivalCity, MAX(ArrivalTime) AS LatestArrival
FROM Flights
GROUP BY ArrivalCity;
...
SELECT AirlineName, AVG(AvailableSeats) AS AvgSeats
FROM Flights
GROUP BY AirlineName
HAVING AvgSeats > 170;
...
SELECT DepartureCity, MIN(DepartureTime) AS EarliestDeparture
FROM Flights
GROUP BY DepartureCity
HAVING COUNT(*) > 3;
...
SELECT DepartureCity, AVG(AvailableSeats) AS AvgSeats
FROM Flights
GROUP BY DepartureCity
HAVING AvgSeats > 120;
